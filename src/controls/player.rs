...
use crate::entities::player::{PlayerCamera, CAMERA_LOOK_POINT, CAMERA_MAX_OFFSET_TRANSLATION, CAMERA_MIN_OFFSET_TRANSLATION};
...
pub fn handle_camera_move(
    ...
    if mouse_btn.pressed(MouseButton::Right) {
        for mouse_event in ev_mouse.read() {
            ...
            let mouse_offset_vec = Quat::from_rotation_y(camera.rotation.y).mul_vec3(Vec3::new(motion.x, 0.0, motion.y));
